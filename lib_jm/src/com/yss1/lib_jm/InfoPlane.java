...
public class InfoPlane {
  ...
  public void setPictFromALL(float x, float y, float l, float h, boolean selected, boolean transparent)
  {
      ...
      float X=rKeeper.getRess().getTextX(x);
      float Y=rKeeper.getRess().getTextY(y);
      float L=rKeeper.getRess().getTextX(l);
      float H=rKeeper.getRess().getTextH(h);
      ...
      FaceState fs = new FaceState(rKeeper).
              init(selected ? "ОбщийВыбран" : transparent ? "ОбщийПрозрачный" : "Общий", new float[]{
                  ...
                  X,        Y + H,
                  X + L,    Y + H});
//                init(selected?"ОбщийВыбран":transparent?"ОбщийПрозрачный":"Общий", new float[]{Lx, Ly - showPixelsY,
//                                                Lx + showPixelsX, Ly - showPixelsY,
//                                                Lx, Ly,
//                                                Lx + showPixelsX, Ly});
      nodePlane.attachChild(ge_pict);
      //ge_pict.setMaterial(rKeeper.getRess().getMaterial("СтолТемный"));
      fs.applyTo(ge_pict);
  }
  
  
  private void setPictCard(Card Cr) {
        ...
        float showPixelsX = pix2show / (float) rKeeper.getRess().getTX_W();
        float showPixelsY = pix2show / (float) rKeeper.getRess().getTX_H();
...
//        FaceState fs = new FaceState(rKeeper).
//                init("ОбщийВыбран", new float[]{Lx, Ly - showPixelsY,
//                                                Lx + showPixelsX, Ly - showPixelsY,
//                                                Lx, Ly,
//                                                Lx + showPixelsX, Ly});
//        nodePlane.attachChild(ge_pict);
//        fs.applyTo(ge_pict);
      ...
      if (!nodePlane.hasChild(ge_pict)) nodePlane.attachChild(ge_pict);
    }
 ...
 public void setText1(String text)
  {
      if (txt1==null) txt1=initTxt(1,1,ColorRGBA.Blue,0.7f*Dy,Dy/3) ;
